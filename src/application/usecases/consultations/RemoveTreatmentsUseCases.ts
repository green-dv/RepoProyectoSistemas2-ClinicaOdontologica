...
export class RemoveTreatmentsConsultationUseCases {
    ...
    async execute(consultationId: number, treatmentIds: number[]): Promise<void> {
        ...
        if (!treatmentIds || treatmentIds.length === 0) {
            throw new Error("Debe proporcionar al menos un tratamiento para remover");
        }
        ...
        await this.consultationRepository.removeTreatments(consultationId, treatmentIds);
    }
}